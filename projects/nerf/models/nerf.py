...
class Model(BaseModel):
    def __init__(self, cfg_model, cfg_data):
        ...
        self.bgcolor = getattr(cfg_data, "bgcolor", 1.0)
        # Define models.
        ...
        self.sample_dists = partial(
            nerf_util.sample_dists,
            dist_range=cfg_model.dist.range,
            intvs=cfg_model.sample_intvs,
        )
        self.sample_dists_from_pdf = partial(
            nerf_util.sample_dists_from_pdf, intvs_fine=cfg_model.sample_intvs_fine
        )
        self.reparametrize_dist = partial(
            nerf_util.reparametrize_dist, param_type=cfg_model.dist.param
        )
        self.get_inverse_depth = partial(
            nerf_util.get_inverse_depth, camera_ndc=cfg_model.camera_ndc
        )
        self.to_full_image = lambda vec, image_size=cfg_data.image_size: vec.moveaxis(
            1, -1
        ).unflatten(dim=-1, sizes=image_size)
    ...
    def _sample_random_rays(self, data):
        ...
        ray_idx = torch.rand(
            batch_size, num_pixels, device=data["pose"].device
        ).argsort(dim=1)[:, : self.num_rays]
        return ray_idx  # [B,R]
    ...
    def inference(self, data):
        ...
        output = self.render_image(
            data["pose"], data["intr"], stratified=False
        )  # [B,N,C]
        ...
        if self.fine_sampling:
            inv_depth_fine = self.get_inverse_depth(
                output["depth_fine"], opacity=output["opacity_fine"]
            )
            output.update(
    ...
    def render_image(self, pose, intr, stratified=False):
        ...
        for center, ray, _ in self.ray_generator(pose, intr, full_image=True):
            ...
            if not self.training:
                ...
                if self.fine_sampling:
                    depth_fine = output_batch["dist_fine"] / ray.norm(
                        dim=-1, keepdim=True
                    )
    ...
    def render_pixels(
        ...
        output = self.render_rays(
            center, ray_unit, stratified=stratified, density_reg=density_reg
        )
        return output
    ...
    def render_rays(
        ...
        with torch.no_grad():
            dists = self.sample_dists(
                ray_unit.shape[:2], stratified=stratified
            )  # [B,R,N,1]
        ...
        if self.fine_sampling:
            with torch.no_grad():
                ...
                dists_fine = self.sample_dists_from_pdf(
                    dists_mid, weights=weights[..., 1:-1, 0]
                )  # [B,R,Nf,1]
            ...
            rays_unit = ray_unit[..., None, :].expand_as(points)  # [B,R,N,3]
            rgbs, densities = self.nerf_fine.forward(
                points, rays_unit, density_reg=density_reg
            )
            weights = render.volume_rendering_weights_dist(
                densities, dists
            )  # [B,R,N,1]
            opacity = render.composite(1.0, weights)  # [B,R,1]
            rgb = render.composite(rgbs, weights)  # [B,R,3]
...
class NeRF(torch.nn.Module):
    ...
    def set_input_dims(self, cfg_model):
        ...
        self.input_3D_dim = (
            3 + 6 * cfg_model.posenc.L_3D if cfg_model.posenc.L_3D else 3
        )
        if cfg_model.view_dep:
            self.input_view_dim = (
                3 + 6 * cfg_model.posenc.L_view if cfg_model.posenc.L_view else 3
            )
        else:
    ...
    def build_model(self, cfg_model):
        ...
        self.mlp_feat = nerf_util.MLPwithSkipConnection(
            layers_feat, skip_connection=cfg_model.mlp.skip
        )
        # RGB prediction.
        layers_rgb = copy.copy(cfg_model.mlp.layers_rgb)
        layers_rgb[0] = cfg_model.mlp.layers_feat[-1] + (
            self.input_view_dim if cfg_model.view_dep else 0
        )
    ...
    def _encode_3D(self, points_3D):
        if self.posenc.L_3D:
            points_enc = nerf_util.positional_encoding(
                points_3D, num_freq_bases=self.posenc.L_3D
            )
    ...
    def _encode_view(self, ray_unit):
        if self.posenc.L_view:
            ray_enc = nerf_util.positional_encoding(
                ray_unit, num_freq_bases=self.posenc.L_view
            )
