...
class Trainer(BaseTrainer):
    def __init__(self, cfg, is_inference=True, seed=0):
        ...
        self.batch_idx, _ = torch.meshgrid(
            torch.arange(cfg.data.train.batch_size),
            torch.arange(cfg.model.rand_rays),
            indexing="ij",
        )  # [B,R]
    ...
    def _compute_loss(self, data, mode=None):
        if mode == "train":
            ...
            image_vec = (
                data["image"].permute(0, 2, 3, 1).view(batch_size, -1, 3)
            )  # [B,HW,3]
            ...
            self.metrics["psnr"] = (
                -10 * torch_F.mse_loss(data["rgb"], image_sampled).log10()
            )
            if self.cfg.model.fine_sampling:
                self.losses["render_fine"] = self.criteria["render_fine"](
                    data["rgb_fine"], image_sampled
                )
                self.metrics["psnr_fine"] = (
                    -10 * torch_F.mse_loss(data["rgb_fine"], image_sampled).log10()
                )
        else:
            ...
            if self.cfg.model.fine_sampling:
                self.losses["render_fine"] = self.criteria["render_fine"](
                    data["rgb_map_fine"], data["image"]
                )
                self.metrics["psnr_fine"] = (
                    -10 * torch_F.mse_loss(data["rgb_map_fine"], data["image"]).log10()
                )
    ...
    def log_wandb_scalars(self, data, mode=None):
        ...
        if "render_fine" in self.losses:
            scalars.update(
                {f"{mode}/PSNR/nerf_fine": self.metrics["psnr_fine"].detach()}
            )
                ...
                images.update(
                    {
                        ...
                        f"{mode}/inv_depth_fine": wandb_image(
                            data["inv_depth_map_fine"]
                        ),
                    }
                )
    ...
    def dump_test_results(self, data_all, output_dir):
        ...
        for key, image_list in results.items():
            ...
            video_writer = skvideo.io.FFmpegWriter(
                video_fname, inputdict=inputdict, outputdict=outputdict
            )
